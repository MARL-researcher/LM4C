...
class SoftPoliciesSelector():
    ...
    def select_action(self, agent_inputs, avail_actions, t_env, test_mode=False):
        # 1. 复制并 Mask Logits
        masked_logits = agent_inputs.clone()
        masked_logits[avail_actions == 0] = -1e10

        if test_mode:
            # === 测试模式：贪婪选择 (Argmax) ===
            # 直接选概率最大的动作
            picked_actions = masked_logits.max(dim=2)[1]
        else:
            # === 训练模式：随机采样 (Sample) ===
            # 建议直接传 logits，比传 probs 更稳
            picked_actions = Categorical(logits=masked_logits).sample().long()
...
class SoftEpsilonGreedyActionSelector():
    ...
    def select_action(self, agent_inputs, avail_actions, t_env, test_mode=False):
        ...
        random_actions = Categorical(probs=avail_actions.float()).sample().long()

        masked_logits = agent_inputs.clone()
        masked_logits[avail_actions == 0] = -1e10
        
        policy_actions = Categorical(logits=masked_logits).sample().long()

        random_numbers = th.rand_like(agent_inputs[:, :, 0])
        
        pick_random = (random_numbers < self.epsilon).long()

        picked_actions = pick_random * random_actions + (1 - pick_random) * policy_actions
