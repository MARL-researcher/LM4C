...
class MPEAnimator(animation.TimedAnimation):
    ...
    def __init__(self,
        ...
        # sizes
        self.agent_sizes = agent_sizes
        self.landmark_sizes = landmark_sizes
    ...
    def save_animation(self, savepath='episode'):
        with contextlib.redirect_stdout(None):
            ...
            self.fig.savefig(savepath+'.jpg', dpi=300)
    ...
    def _draw_frame(self, frame):
        ...
        for n in range(self.n_agents):
            self._episode_update(self.agent_positions[n], self.lines_episode[line_episode], frame, 1)
            line_episode += 1
        ...
        # update agents trajectory lines
        for n in range(self.n_agents):
            self._episode_update(self.agent_positions[n], self.lines_episode[line_episode], max(0,frame-1), self.lags)
            line_episode += 1
    ...
    def _init_episode_animation(self, ax):
        ...
        # 2. agent trajectory
        # 3. landmark real
        agent_heads = [ax.plot([],[],'o',color=self.agent_colors[a], alpha=0.8,markersize=self.agent_sizes[a])[0] for a in range(self.n_agents)]
        traj_points = [ax.plot([],[],'o',color=self.agent_colors[a], alpha=0.2,markersize=self.agent_sizes[a]/1.5)[0] for a in range(self.n_agents)]
        traj_lines = [ax.plot([],[],'-',color=(0.25, 0.41, 0.88), alpha=1,linewidth=1.5)[0] for _ in range(self.n_agents)]
        landmarks = [ax.plot([],[],'o',color=self.landmark_colors[l], alpha=0.8,markersize=self.landmark_sizes[l])[0] for l in range(self.n_landmarks)]
        
        lines = agent_heads + traj_points + traj_lines + landmarks
