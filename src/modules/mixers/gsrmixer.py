...
from components.soft_attention import SoftAttention

class GSRMixer(nn.Module):
    def __init__(self, args, abs=True):
        super(GSRMixer, self).__init__()

        self.args = args
        self.n_agents = args.n_agents
        self.embed_dim = args.mixing_embed_dim
        self.input_dim = self.state_dim = args.rnn_hidden_dim
        ...
        self.att_layer = SoftAttention(args)

        if getattr(args, "use_orthogonal", False):
            for m in self.modules():
                orthogonal_init_(m)

    def forward(self, qvals, trajectory, true_states, output_k=False, output_b=False):

        # construct the attention state
        att_states = self.att_layer(trajectory, true_states)

        # reshape
        b, t, _ = qvals.size()
        
        qvals = qvals.reshape(b * t, 1, self.n_agents)
        states = att_states.reshape(-1, self.state_dim)
        ...
        if output_k and output_b:
            k_params = self.k(states)
            b_params = self.b(states)
            return y.view(b, t, -1), k_params.view(b, t, -1), b_params.view(b, t, -1)
        elif output_k:
            k_params = self.k(states)
            return y.view(b, t, -1), k_params.view(b, t, -1)
        elif output_b:
            b_params = self.b(states)
            return y.view(b, t, -1), b_params.view(b, t, -1)
        else:
            return y.view(b, t, -1)
    ...
    def k(self, states):
        bs = states.size(0)
        w1 = self.pos_func(self.hyper_w1(states)).view(-1, self.n_agents, self.embed_dim)
        w2 = self.pos_func(self.hyper_w2(states)).view(-1, self.embed_dim, 1)
        k = th.bmm(w1, w2).view(bs, -1, self.n_agents)
    ...
    def predict_cf_value(self, qval, trajectory, state):
        pass
